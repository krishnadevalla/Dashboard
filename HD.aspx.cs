...
public partial class HD : System.Web.UI.Page
{
    ...
    void Write2DB()
    {
        ...
        String cmdString1 = "Insert into Incident (CreateDate, fName, lName, " +" phone, email, sysName, IP, OS, FF, Description, Status, ResponseTime, " + " ResolutionTime) Values " + "('" + DateTime.Now + "','" + tbUserFirst.Text + "','" + tbUserLast.Text + "','" + tbUserPhone.Text + "','" + tbUserEmail.Text + "','" +
            tbSysName.Text + "','" + tbIP.Text + "','" + ddOS.SelectedItem.Text + "','" + rbFF.SelectedItem.Text + "' ,'" + tbProblem.Text + "' ,'" + "OPEN" + "' ,'" + rbPriority.Text + "' ,' ')";
        conn = new SqlConnection(@"Data Source= omisbi3.niunt.niu.edu;Initial Catalog=z1747241;User ID=OMIS675FA;Password=omis675 ");
